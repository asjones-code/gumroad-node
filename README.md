...
The REST API documentation can be found on [flowoid.com](https://flowoid.com). The full API of this library can be found in [api.md](api.md).
...
async function main() {
  const product: Response = await client.products.list();
}
...
async function main() {
  const product = await client.products.list().catch(async (err) => {
    if (err instanceof Gumroad.APIError) {
...
add the following import before your first import `from "Gumroad"`:
...
- Deno v1.28.0 or higher, using `import Gumroad from "npm:gumroad-stainless"`.
- Bun 1.0 or later.
- Cloudflare Workers.
- Vercel Edge Runtime.
